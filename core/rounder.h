...
namespace symfpu {
  ...
  // Handles the overflow and underflow conditions
  template <class t>
  floatWithStatusFlags<t> rounderSpecialCases_flagged (const typename t::fpt &format,
					const typename t::rm &roundingMode,
					const floatWithStatusFlags<t> &roundedResult,
					const typename t::prop &overflow,
					const typename t::prop &underflow,
					const typename t::prop &isZero)
  {
    ...
    return ITE(isZero,
      floatWithStatusFlags<t>(zero),
      ITE(underflow,
        floatWithStatusFlags<t>::makeUnderflow(ITE(returnZero, zero, min)),
        ITE(overflow,
          floatWithStatusFlags<t>::makeOverflow(ITE(returnInf, inf, max)),
          roundedResult)));
  }
...
template <class t>
  floatWithStatusFlags<t> customRounder_flagged (const typename t::fpt &format,
				  const typename t::rm &roundingMode,
				  const unpackedFloat<t> &uf,
				  const customRounderInfo<t> &known) {
...
template <class t>
  floatWithStatusFlags<t> rounder_flagged (const typename t::fpt &format,
			    const typename t::rm &roundingMode,
			    const unpackedFloat<t> &uf) {
  typedef typename t::prop prop;
  customRounderInfo<t> cri(prop(false), prop(false), prop(false), prop(false), prop(false));  // Default is to know nothing

  return customRounder_flagged(format, roundingMode, uf, cri);
 }
